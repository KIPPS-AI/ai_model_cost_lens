...
export function TaskConfig({ config, onChange }: Props) {
	const toggleComponent = (comp: "llm" | "stt" | "tts" | "streaming") => {
		...
		const next = has ? config.aiComponents.filter((c) => c !== comp) : [...config.aiComponents, comp];
	...
	const isImageOrVideo = config.taskType === "image-generation" || config.taskType === "video-generation";
		...
		<div className="space-y-5">
			<div className="space-y-1.5">
				...
				{(() => {
					...
					return found ? <p className="text-xs text-muted-foreground">{found.description}</p> : null;
				})()}
			</div>
			...
			{!isImageOrVideo && (
				<div className="space-y-2">
					<Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">AI Components</Label>
					<div className="divide-y divide-border rounded-md border border-border">
						{AI_COMPONENTS.map((comp) => {
							...
							return (
								<label key={comp.value} htmlFor={`comp-${comp.value}`} className="flex cursor-pointer items-center gap-3 px-3.5 py-3 hover:bg-muted/40 transition-colors first:rounded-t-md last:rounded-b-md">
									<Checkbox id={`comp-${comp.value}`} checked={checked} onCheckedChange={() => toggleComponent(comp.value)} />
									<div className="min-w-0 flex-1">
										<div className="flex items-center gap-2">
											<p className="text-sm font-medium text-foreground">{comp.label}</p>
											{config.taskType !== "custom" && relevant && <span className="rounded bg-primary/10 px-1.5 py-0.5 text-xs text-primary">recommended</span>}
										</div>
			...
			{isImageOrVideo && (
				<div className="rounded-md border border-border bg-muted/40 px-3.5 py-3">
					<p className="text-xs text-muted-foreground">{config.taskType === "image-generation" ? "Configure the image model and images-per-request below. Token-based LLM components are not used." : "Configure the video model and seconds-per-request below. Token-based LLM components are not used."}</p>
				</div>
			)}
		</div>
	);
